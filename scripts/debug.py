...
with conn.cursor() as cur:
    cur.execute("SELECT * FROM historical_transactions WHERE phase='scoring' AND is_anomaly=True AND 'amount_spike' = ANY(anomaly_types) LIMIT 1")
    row = cur.fetchone()
    col_names = [d[0] for d in cur.description]
    row_dict = dict(zip(col_names, row))
    payload = row_to_payload(row_dict)
    
    cur.execute(f"SELECT profile_data FROM card_profiles WHERE card_id_hash='{payload['card_id_hash']}'")
